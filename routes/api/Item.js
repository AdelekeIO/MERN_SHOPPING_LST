...
router.post('/', (req, res)=>{
    let name = req.body.name;
      ...
      con.query("INSERT INTO `items` (`id`, `name`, `date_added`) VALUES (NULL, '"+name+"', current_timestamp())", function (err, result) {
        if (err) throw err;
        console.log("1 Row Inserted");
        
        con.query("SELECT id, name FROM items ORDER BY id DESC LIMIT 1", function (err, result) {
          if (err) throw err;
          res.json(result[0]);
        });
      });
